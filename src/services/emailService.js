...
<body style="margin:0;padding:0;font-family:Inter,Arial,sans-serif;background:#F8FAFC;">
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;">
    <tr>
      <td style="background:#0D1B2A;padding:28px 32px;border-radius:12px 12px 0 0;">
        <div style="color:#1A56DB;font-size:22px;font-weight:800;letter-spacing:-0.5px;">⚡ ComplianceAI</div>
    ...
    <tr>
      <td style="background:#ffffff;padding:32px;">
        ...
        <div style="background:${urgencyColor}15;border:2px solid ${urgencyColor};border-radius:12px;padding:20px;text-align:center;margin-bottom:24px;">
          <div style="color:${urgencyColor};font-size:36px;font-weight:900;letter-spacing:-1px;">${urgencyLabel}</div>
        ...
        <a href="${renewalUrl}" style="display:block;background:#1A56DB;color:#ffffff;text-decoration:none;text-align:center;padding:16px;border-radius:10px;font-size:16px;font-weight:700;margin-bottom:24px;">Renew Now →</a>
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [to],
      subject: `⚠️ ${licenseName} — ${urgencyLabel}`,
      html,
    }),
  });
