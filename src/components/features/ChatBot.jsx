...
export default function ChatBot() {
  ...
  const businessCtx = isDemo ? demoBusiness : null;
  ...
  const sendMessage = async (text) => {
    ...
    let aiText = '';
    await chatWithAI(text, businessCtx, messages, (chunk) => {
      aiText += chunk;
      setMessages(prev => {
        const next = [...prev];
  ...
  return (
    <>
      {/* Floating button */}
      <div className="fixed bottom-20 right-4 lg:bottom-6 z-40">
        <AnimatePresence>
          {!open && (
            <motion.button
              initial={{ scale: 0 }}
              ...
              className="relative w-14 h-14 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-full shadow-lg flex items-center justify-center text-white"
            >
              <MessageCircle size={24} />
              {unread && (
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full border-2 border-white text-[9px] flex items-center justify-center font-bold">1</span>
              )}
            </motion.button>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {open && (
            ...
            >
              ...
              {/* Messages */}
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 chat-scroll bg-gray-50">
                {messages.length === 0 && (
